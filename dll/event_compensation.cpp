...
RadarSoundSpec radar_sound_from_event(std::string_view event_name,
  ...
  if (event_name == "player_footstep") {
    // Surface-specific VSND metadata varies in live play. 1100 is the neutral
    // fallback used only when the runtime sound emitter supplied no native row.
    return {RadarSoundKind::footstep, 1100, 0.5f, true};
  }
  if (event_name == "player_jump") {
    // This demo retains player_jump but has no stable player_land event. The
    // pinned runtime's native VSND ingress repeatedly reports 204/548-unit
    // movement cues; use the smaller jump cue only when no exact native row
    // arrives during the priority window.
    return {RadarSoundKind::utility, 204, 0.1f, false};
  }
  if (event_name == "weapon_zoom") {
    // Recorded player_sound samples put the scope click in the 580..620 band.
  ...
  if (is_utility(weapon)) {
    // Insight Agent uses a slightly longer 160 ms throw/release cue. The
    // radius and lifetime are still consumed by CS2's native radar renderer.
